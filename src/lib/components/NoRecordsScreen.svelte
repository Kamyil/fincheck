...
{#if browser}
    ...
    -->
            <td use:fullWidthColspan transition:fade>
                <div class="p-16 grid place-items-center text-xs text-gray-500">Brak wyników</div>
            </td>
        </tr>
    {:else}
        <!-- TODO: When app will be migrated to Svelte 5, then move this div to the snippet and reuse it in {#if insideTable} block -->
        <div class="p-16 grid place-items-center text-xs text-gray-500" transition:fade>Brak wyników</div>
    {/if}
{/if}
