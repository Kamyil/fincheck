...
<div use:portal id="notifications-slide-track" class="fixed bottom-10 right-10 z-[99]">
    {#each $toasts as { id, data } (id)}
        <div
            ...
            use:melt={$content(id)}
            class={twMerge("bg-white backdrop-blur-xl relative p-5 rounded-md shadow-2xl border z-[99] my-5 opacity-90", classes, TypeToColorMap[data.type])}
        >
            <div>
                <div class="p-2">
                ...
                <button use:melt={$close(id)} aria-label="close notification" class="absolute z-50 top-2 right-2 cursor-pointer" class:text-black={data.type === "normal"}>
                    <X size={14} />
                </button>
            </div>
        </div>
