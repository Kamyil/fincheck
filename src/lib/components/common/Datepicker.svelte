<script lang="ts">
    ...
    interface Props {
         ...
         */
        error?: boolean;
        onChange?: ({ previous, next }: { previous: DateValue | undefined; next: DateValue | undefined }) => void;
    }
    ...
    function currentValueObject() {
        ...
        return new CalendarDate(new GregorianCalendar(), getYear(date), getMonth(date) + 1, getDate(date));
    }

    const {
        elements: { calendar, heading, grid, cell, prevButton, nextButton, trigger, content, segment, field },
...
<div class="relative flex items-center" use:melt={$field}>
    <input {id} {name} type="hidden" bind:value={$valueStore} onchange={(e) => (value = e.currentTarget.value)} />

    <div class={twMerge("rounded-md p-2 text-gray-900 transition-all duration-150 ", classes)} class:error>
    ...
    <button type="button" class="w-6 h-fill absolute right-1" use:melt={$trigger}>
        <CalendarIcon classes="text-[1rem]" />
    </button>
</div>

{#if $open}
    <div transition:fly={{ y: -30, duration: 200 }} use:melt={$content} class={twMerge("z-[60]", datepickerClasses)}>
...
<style lang="postcss">
    ...
    [data-melt-datefield-label] {
        @apply select-none font-medium text-stone-800;
    }
    ...
    [data-melt-calendar-prevbutton] {
        @apply rounded-lg p-1 transition-all hover:bg-primary/20;
    }
    ...
    [data-melt-calendar-cell] {
        @apply flex h-6 w-6 cursor-pointer select-none items-center justify-center rounded-lg p-4 hover:bg-primary/90 hover:text-white focus:ring focus:ring-primary;
    }
