<script lang="ts">
	...
	import {
		...
		type Filter,
		gotoFilters,
		type PaginationFiltersDefinition
	...
	interface Props {
		...
		total?: number;
		filters: Filters<
			Record<string, PaginationFiltersDefinition['page'] | PaginationFiltersDefinition['perPage']>
		>;
	...
	const perPageFilter = $state(
		filters[perPageFilterName] as Filter<PaginationFiltersDefinition['perPage']>
	);
	...
	run(() => {
		if (pageFilter.value * perPageFilter.value > total + perPageFilter.value) {
			...
			gotoFilters($page.url, {
				...filters,
				[perPageFilterName]: perPageFilter,
...
{#if total && total > MINIMUM_PER_PAGE_ITEMS_AMOUNT}
	...
	<div transition:slide class={twMerge('mr-5 flex flex-row', classes)}>
		...
		{#each _range(pageFilter.value - pagesBeforeCurrent, 1 + pageFilter.value + pagesAfterCurrent) as pageNumber}
			<PaginationItem
				size="medium"
				active={pageNumber === pageFilter.value}
