...
<div>
	{#if label}
		<label
			for={id}
			class="mt-2 mb-1 ml-2 block text-xs font-medium text-zinc-500"
	...
	<textarea
		...
		bind:value
		{rows}
		class={twMerge('h-full w-full rounded text-xs', variants[variant], classes)}
	...
	{#if letterTracker}
		{#if dunkMode}
			<span
				class="relative bottom-10 float-right p-2 text-xs"
				style="
		...
		{:else}
			<span class="float-right mb-4 p-2 text-xs" class:text-red-600={length > letterTracker.limit}>
				{length} / {letterTracker.limit}
			</span>
		{/if}
	{/if}
