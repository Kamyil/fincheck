<script lang="ts">
	...
	interface Props {
		...
		errors:
			| (Record<number, string[] & { _errors?: string[] }> & { _errors?: string[] })
			| undefined;
	}
		...
		files.forEach((f, idx) => {
			if ('file' in f) {
				if (newFilesIdx in currentErrors && currentErrors[newFilesIdx]) {
					...
					newFilesErrorMap.set(
						idx,
						'_errors' in errorValue ? (errorValue._errors ?? []) : errorValue
					);
				}
...
<div class={classes}>
	...
	<ul transition:slide class="w-full">
		{#each files as file, idx}
			...
			<li
				transition:slide
				class="relative my-2 flex w-full items-center justify-center rounded-md border-1 bg-white p-2 shadow-md"
			>
				<span class="w-11/12 text-sm text-stone-700">
					{#if 'path' in file && 'isDeleted' in file}
						<a
							href={`/api/storage/${file.path}`}
							download={file.name}
							class="underline"
				...
				{#if 'isDeleted' in file}
					{#if file.isDeleted}
						<button
							type="button"
							class="absolute right-1 mr-2 flex items-center justify-center text-red-600 hover:fill-red-800"
							onclick={() => {
					...
					{:else}
						<button
							type="button"
							class="absolute right-1 mr-2 flex items-center justify-center"
							onclick={() => {
								file.isDeleted = true;
							}}
							title="Usuń"
