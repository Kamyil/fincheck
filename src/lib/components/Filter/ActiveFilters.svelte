        ...
        {#if filter.value && !excludeKeys.includes(key)}
            {#if filter.separator}
                {#each splitFilterValue(filter, filter.value) as splitValue}
                    ...
                    <div class="h-8 w-fit p-2 text-xs font-normal cursor-pointer flex items-center gap-2 rounded-lg transition-all active:scale-95 text-primary border border-primary">
